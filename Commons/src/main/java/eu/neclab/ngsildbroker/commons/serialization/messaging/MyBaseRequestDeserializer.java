...
public class MyBaseRequestDeserializer {
	...
	public BaseRequest deserialize(String body) throws IOException, JacksonException, IllegalArgumentException, IllegalAccessException {
		BaseRequest result = new BaseRequest();
		final char[] chars = (char[]) field.get(body);
	    final int len = chars.length;
	    
	    for (int i = 2; i < chars.length; i++) {
	    	switch(chars[i]) {
	    	//case AppConstants.ZIPPED_SERIALIZATION_CHAR: 
	    	}
		}
		return result;
	}
